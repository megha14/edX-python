...
def getWordScore(word, n):
    ...
    for letter in word:
        if letter in SCRABBLE_LETTER_VALUES.keys():
            sum_of_letters += SCRABBLE_LETTER_VALUES[letter]
    if len(word)==n:
        return (sum_of_letters*len(word))+50
    else:
        return (sum_of_letters*len(word))
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while(calculateHandlen(hand) > 0):
        ...
        # Otherwise (the input is not a single period):
            ...
            print '"'+str(user_input)+'"'+" earned "+str(getWordScore(user_input,n))+" points. Total: "+str(total_score)
            print ""
            # Update the hand 
