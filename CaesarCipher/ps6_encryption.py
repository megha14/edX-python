...
WORDLIST_FILENAME = "C:\edx_py\PS6\words.txt"
...
def getStoryString():
    ...
    """
    return open("C:\edx_py\PS6\story.txt", "r").read()
...
def buildCoder(shift):
    ...
    i=0
    for char in strUpper:
        if char not in dictList.keys():
            dictList[char] = strUpper[i+shift]            
        if i==len(strUpper)-shift-1:
            i=-shift
        else:
            i+=1
    for char in strLower:
        if char not in dictList.keys():
            dictList[char] = strLower[i+shift]            
        if i==len(strUpper)-shift-1:
            i=-shift
        else:
            i+=1         
